...
The suite requires a real Postgres and fails rather than skips when it is missing, because a suite that skips its own dependency keeps CI green while testing nothing, and CI greps its own output for skip markers to make sure that cannot regress silently. A second CI job reinstalls XGBoost, regenerates the fixtures from scratch and re-runs the parity tests, comparing predictions rather than files — retraining produces different trees, and the claim under test is that the Go scorer agrees with XGBoost, not that training is reproducible. Coverage is 90.4% of internal packages, and every command and number in the README, including the drift table and the batching figures, was run against a live server before being written down. So is the list of known limitations: only gbtree models, no eviction so the working set must fit in memory, drift on the first output only for multi-class models, a guard that catches errors rather than quality, a login flow built for a terminal rather than a browser session, so the server has no cookies and no sign-in page, a credential file that is worth more now that it holds a refresh token, renewal that needs the provider reachable where a static token would not, static-token revocation only as fast as the next reload, group membership read from the token rather than the directory, a rate limit keyed on client address so a NAT shares one bucket, and no volumetric defence, which still belongs at a gateway.
