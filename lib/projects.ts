...
The suite requires a real Postgres and fails rather than skips when it is missing, because a suite that skips its own dependency keeps CI green while testing nothing, and CI greps its own output for skip markers to make sure that cannot regress silently. A second CI job reinstalls XGBoost, regenerates the fixtures from scratch and re-runs the parity tests, comparing predictions rather than files — retraining produces different trees, and the claim under test is that the Go scorer agrees with XGBoost, not that training is reproducible. Coverage is 89.9% of internal packages, and every command and number in the README, including the drift table and the batching figures, was run against a live server before being written down. So is the list of known limitations: only gbtree models, no eviction so the working set must fit in memory, drift on the first output only for multi-class models, a guard that catches errors rather than quality, browser sessions held in memory so a restart signs everybody out, deploy actions with no approval step beyond the person clicking, a dashboard that polls rather than streams, a credential file that is worth more now that it holds a refresh token, renewal that needs the provider reachable where a static token would not, static-token revocation only as fast as the next reload, group membership read from the token rather than the directory, a rate limit keyed on client address so a NAT shares one bucket, and no volumetric defence, which still belongs at a gateway.
...
Tokens rotate without restarting the process. They are read from a file that is re-read on SIGHUP and on a timer, and the set is swapped through an atomic pointer rather than mutated — the read side takes no lock, so rotation costs authentication nothing, and a request sees exactly one generation of the set. Mutating a shared map in place would let a request arriving mid-rotation see a state where the new token is present and the old one is already gone, which is the window rotation exists to avoid; measured on a running server, four hundred requests spanning forty live reloads all returned 200 under the same process. The procedure is two steps with both credentials valid in between, because a single-step swap breaks every client that has not been updated yet, which is exactly why rotation gets deferred until a credential is years old. A bad rotation is not an outage either: the file is parsed and validated before anything is swapped, so a malformed entry or a file briefly missing mid-rewrite leaves the running set in force, logs, and increments a counter that has an alert on it — a failed reload is invisible from outside, since the server keeps serving and the rotation simply did not happen. It is a file rather than an endpoint that mints tokens on purpose: an issuing API turns any admin compromise into permanent access, because the attacker issues a credential of their own and keeps it after the stolen one is revoked. Entries can carry an RFC 3339 deadline, enforced per request rather than at load time, so a credential stops working whether or not anybody reloads.

People authenticate with OIDC rather than a shared secret, which is what makes the audit log answer who rather than which credential — the readable identity and the stable subject are both recorded, since an email is what makes a line legible without a directory lookup and a subject is what still joins correctly after somebody's surname changes. Service tokens and user logins coexist, told apart by shape, because a service scoring a thousand requests a second wants a credential it holds and a person changing what serves traffic should be named. The decision I would most want to defend here is not writing the JWT verification myself, which sits oddly in a repository whose premise is building things from the wire up. The difference is what a bug costs: a mistake in the scorer produces a wrong number that a differential test against XGBoost catches, while a mistake in JWT verification is a total authentication bypass — alg:none, RSA-versus-HMAC confusion, an unchecked key id — whose failure mode is that everything works perfectly while anybody can forge an admin identity. So the tests verify the wiring rather than the cryptography: a real key, a real JWKS endpoint over HTTP, real signed tokens, and assertions that a wrong-key signature, swapped claims, an unsigned token, an expired one and one minted for another audience are each refused. The audience check is mandatory with no permissive setting, because skipping it is the confused-deputy bug where a token a provider issued for a different service becomes a valid credential here. Writing the end-to-end test also caught that a user in no mapped group was being refused with 401, which is wrong and confusing — their signature checked out and their provider vouched for them, so re-authenticating sends a real employee round a loop against a provider that works when the fix is somebody adding them to a group.

Signing in is the RFC 8252 native-application flow, and every part of its shape follows from what a command-line tool can safely do. There is no client secret, because one shipped inside a binary is a string every user can read out of it, so the client is public and PKCE takes the secret's place — the test provider recomputes the challenge from the verifier and refuses a mismatch, which means the CLI cannot ship without PKCE and still pass. The redirect goes to 127.0.0.1 rather than localhost, since localhost resolves through DNS and a hostile resolver delivers the authorization code to somebody else's machine. A state parameter is checked before the code is touched, or anyone who can make a browser load a URL completes a login of their choosing and leaves the victim holding their session, and a nonce binds the token to this particular login. The credential lands on disk at mode 0600 in a 0700 directory, both set explicitly rather than left to the umask, with an existing directory tightened rather than trusted because MkdirAll leaves an existing mode alone. Smoke-testing the flow found a bug I would not have predicted: leaving the OAuth client-authentication style unset makes x/oauth2 probe for the server's preference by trying one and retrying with the other, and since an authorization code is single-use the probe burns it — so the error surfaced is the second attempt's invalid_grant, pointing at the code rather than at whatever actually went wrong first. Pinning it to the body parameter is both what RFC 6749 specifies for a public client and the end of that failure mode: one token request per login where there had been two.

Sessions renew themselves rather than expiring out from under somebody mid-task, which matters because the alternative in practice is that people reach for a permanent static token instead of an hour-long one. Renewal is proactive rather than a reaction to a rejection, since reacting means the first call after expiry always fails once and teaches people that the tool works on the second try; the margin covers clock skew and the time a request spends in flight, because a token with two seconds left is not usable even though it has not technically expired. Refresh-token rotation is honoured, which matters because keeping only the original token works perfectly until the provider retires it and then fails days later with no obvious cause. The subtlety I am most pleased to have handled is concurrency: two commands at once both see a stale token and both refresh, and against a rotating provider the second invalidates the first, so whichever writes last leaves a credential the other already broke — a login that mysteriously dies when somebody runs two things in parallel. Refresh happens under a file lock that re-reads the credential inside it, and eight concurrent workers now produce exactly one refresh. Adding a refresh token also means adding a longer-lived secret to disk, so logout now calls the provider's RFC 7009 revocation endpoint rather than only deleting a file — verified by restoring the deleted credential afterwards and watching the provider refuse it, which is the difference between a session being dead and merely forgotten locally.

A browser can sign in and hold a session, and the cookie carries an opaque random id rather than a signed identity — a signed cookie cannot be revoked before it expires, because the server has nothing to delete, and revocation is most of what makes handing out sessions safe. Every cookie attribute closes a specific hole and each has a test: HttpOnly so an XSS bug elsewhere cannot read the session, Secure so it never crosses plaintext, SameSite so a cross-site form post does not carry it, and no Domain so one compromised subdomain cannot read it. Reads need only the cookie while state-changing methods also need a CSRF token echoed in a header — SameSite already closes that door, and the token is the second lock on it, worth having because SameSite is a browser behaviour that an old browser or a header-stripping proxy defeats while a token the attacker cannot read is not defeated by any of that. The property I was most careful about is that a session is never a weaker front door: the identity token is verified through the same path an API request takes, so a cookie cannot carry an identity a bearer token could not, and a bearer token wins when both are present because a client that sent a credential meant it. Session ids are minted only after authentication, which makes fixation impossible, and the redirect target is validated rather than trusted — an open redirect on a login endpoint is worth more than most, since the victim authenticates for real at their real provider and is then bounced somewhere the attacker controls, and a protocol-relative //host is rejected explicitly because a browser reads it as another origin while a naive prefix check reads it as a local path.

There is a dashboard at last, and the decision worth defending is that it is server-rendered HTML with no JavaScript at all rather than a single-page app. html/template does contextual auto-escaping, which is the XSS defence, and that matters more than it would have earlier in the project because the server now hands out session cookies — a script injected into one of these pages would run with somebody's session. Having no JavaScript is also what lets the Content-Security-Policy be default-src 'none' with no script-src at all, which turns "everything is escaped correctly" from a claim into something the browser enforces independently, and a test asserts the pages contain no script tags so a future addition fails loudly rather than being silently blocked. It keeps the binary a single self-contained artifact with no bundler and no node in CI for a Go serving platform. Deploying, canarying, shadowing and rolling back are available from the page, and every one is two steps — a plan that says exactly what will change, then an apply — because a single button that immediately moved traffic would be the wrong shape for an operation that decides which model answers every request and is being triggered by a mouse rather than a reviewed script. Apply refuses if the policy changed between the plan and the confirmation, since otherwise the later click wins by arriving later and whoever's change was discarded has no way to know. That is also what the CSRF form field is for: a page with no JavaScript cannot set a header, so the token comes back in the body instead, and only form-encoded bodies are parsed for it because calling ParseForm on a JSON request would consume the body the handler is about to decode and would have silently broken every API POST. Both surfaces share one policy-application path, so a version that cannot load is refused identically and a shadow is carried across the same way — two code paths that both change what serves production traffic would be two places for the rules to drift, and the one with fewer checks becomes the way in. A browser gets a redirect to sign in where an API client gets a 401, which is the one place the HTML surface should differ: a browser handed a bare 401 has nowhere to go, and an API client handed a redirect follows it and tries to parse a login page as JSON. Rendering it against real traffic surfaced something I liked: a ten percent canary shows a mean batch of 3.1 rows against the incumbent's 26.3, because the same batching window fills more slowly at a tenth of the volume.
...
A third turned up when I made the compose stack's host ports configurable and ran the same smoke test against non-default ones. The issuer both services were built with was a hardcoded localhost:8081, so with the port moved the provider minted tokens claiming an issuer on a port nothing was listening on, and the authorization service — which compares the iss claim against the tenant's expected issuer, exactly as it should — refused them. Static checks were perfectly happy; only running the flow caught it. The issuer is what a token asserts to an outside caller rather than an internal address, so it now follows the published port, and the two services still derive it from one value because a token whose issuer disagrees with the verifier's is indistinguishable from a forged one.`,
    stack: [
      "Java 21",
      "Spring Boot",
      "OAuth 2.0",
...
A third failure was more interesting, because the bug was in the test rather than the code. The helper every test used to write a key failed on the first error from Propose, which quietly made all of them assert something Raft never promises: a proposal accepted by a leader that is deposed before its entry commits returns ErrLeadershipLost, and that is an ordinary outcome rather than a defect — the HTTP API already reports it as a 503 precisely so the caller can repeat the write. With a 5ms tick and an eight-tick election timeout, a loaded CI runner only has to stall the leader for 40ms for a follower to campaign, and the snapshot-catch-up test eventually died on exactly that while the code under test behaved as designed. The helper now re-resolves the leader and retries, which is what a real client does, and retrying is safe because a Put is idempotent. The tests that deliberately assert leadership loss call Propose directly and were left alone, so that error path keeps its coverage.`,
    stack: [
      "Go",
      "Raft",
      "Distributed Systems",
